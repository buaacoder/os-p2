...
tid_t process_execute(const char *file_name) {
    ...
    if (tid == TID_ERROR) {
        ...
        sema_down(&son->get_msg);
        list_push_back(&cur->son_list, &son->son_elem);
    }
...
static void start_process(void *file_name_) {
    ...
    *p = 0;
    ...
    if (cur->father != NULL) {
        ...
        sema_down(&cur->get_msg);
        // printf("%d\n",cur->tid);
        // printf("hhhhhhhhhhhhhhhhhhhhhh33\n");
    }
    palloc_free_page(file_name);
    // printf("hhhhhhhhhhhhhhhhhhhhhh44\n");
...
int process_wait(tid_t child_tid UNUSED) {
    ...
    // timer_sleep(200);
    // return -1;
    struct thread *son = get_thread_by_sid(child_tid);
    int ret = -1;
    // printf("wait:%d\n",son->tid);
    if (son == NULL || son->saved == true || son->status == THREAD_DYING) {
        return -1;
    } else {
        // printf("wait:%d\n",son->tid);
        son->waited = true;
...
void process_exit(void) {
    ...
    if (!list_empty(mmap_files)) {
        struct mmap_info *mmapinfo;
        while (!list_empty(mmap_files)) {
            mmapinfo = list_entry(list_pop_front(mmap_files), struct mmap_info,
                                  list_elem);
            for (int i = 0; i < mmapinfo->page_num; i++) {
                uninstall_file(cur->sup_page_table,
    ...
    struct list_elem *l_elem, *next_elem;
    for (l_elem = list_begin(&cur->file_list);
         l_elem != list_end(&cur->file_list); l_elem = next_elem) {
        struct file_node *fn = list_entry(l_elem, struct file_node, elem);
        next_elem = list_next(l_elem);
        file_close(fn->f);
        free(fn);
    }
    if (cur->father != NULL) {
    ...
    destroy_page_table(cur->sup_page_table);
...
bool load(const char *file_name, void (**eip)(void), void **esp) {
    ...
    t->sup_page_table = create_page_table();
...
static bool setup_stack(void **esp) {
    ...
    kpage = get_frame(PHYS_BASE - PGSIZE, PAL_USER | PAL_ZERO);
    // printf("kpage = %d\n", kpage);
